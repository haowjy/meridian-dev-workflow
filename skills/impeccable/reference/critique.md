...
CLI scan:
```bash
node skills/impeccable/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node skills/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
