...
@mockup-gen — fast throwaway mockups using the project's real components and design system. Not production code — exists for rapid visual iteration with the user. One per iteration round. Spawned by @frontend-dev, not by impl-orchestrator.

@imagegen — native image generation. UI concept mockups, visual explorations, icons, reference imagery. Usually spawned on explicit user request — image generation is expensive. Spawned by @frontend-dev.
